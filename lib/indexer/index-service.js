...
function getIndexBatchSize() {
  return Math.max(2, Math.min(6, Math.floor(getCpuCount() / 2)));
}

function getFaceBatchSize() {
  return Math.max(1, Math.min(3, Math.floor(getCpuCount() / 4) || 1));
}

function getVisualBatchSize() {
  return Math.max(1, Math.min(2, Math.floor(getCpuCount() / 6) || 1));
}
...
async function ensureAnalysisProxy(proxyDir, file, ctx = null) {
  ...
  try {
    ...
    await fsp.writeFile(proxyPath, resized.toJPEG(85));
    return proxyPath;
  } catch (error) {
...
async function resolveFileMetadata(db, file, existing, { thumbDir, analysisDir, heicDir, queries, runId }) {
    ...
    if (file.mediaType === 'image' && (!thumbnailPath || !fs.existsSync(thumbnailPath))) {
      try {
        ...
        if (img) {
          ...
          await fsp.writeFile(thumbPath, thumb.toJPEG(80));
...
async function runIndexing(db, app, options = {}) {
  ...
  for (let index = 0; index < toProcess.length; index += batchSize) {
    ...
    await Promise.all(batch.map(async (file) => {
      ...
      try {
        const { mediaId, decodablePath, thumbnailPath, ctx, needsVisualIndexing, needsFaceIndexing, needsEmbedding, hasLocation } = result;
...
async function generateFaceThumbnail(filePath, box, ctx = null, decodablePath = null) {
  try {
    ...
    if (ctx && ctx.image) {
      ...
      const { nativeImage } = require('electron');
      const loadPath = decodablePath || filePath;
      img = nativeImage.createFromPath(loadPath);
    ...
    await fsp.writeFile(thumbPath, resized.toJPEG(85));
    return thumbPath;
  } catch (err) {
...
async function processMetadataBatches(db, app, scannedFiles, options = {}) {
  ...
  for (let index = 0; index < toProcess.length; index += batchSize) {
    ...
    await Promise.all(batch.map(async (file) => {
      ...
      const result = await resolveFileMetadata(db, file, existing, { thumbDir, analysisDir, heicDir, queries, runId });
      try {
        const { mediaId, decodablePath, thumbnailPath, ctx, needsVisualIndexing, needsFaceIndexing, needsEmbedding } = result;
